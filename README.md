...
Зеленой стрелкой отмечена СКРЫТАЯ папка *(у меня включено отображение скрытых файлов)* с именем "**.github**", в которой содержится полная информация о том, откуда именно скачан дистрибутив, его версия и все, что нужно знать системе "git", установленной на компьбтере локально.
