...
Информация: Ошибка "torch\lib\fbgemm.dll"
Проблема связана с некоторой несовместимостью между версией CUDA и версией PyTorch в Windows. 

Проявляется (протестировано на свежей винде 10) на 3.11 питоне, на 3.10 не проявляется. Проблема в TORCH, т.к. проявляется не только в SD-интерфейсах, но и в любой конфигурации. когда требуется установка связки TORCH+CUDA Для избежания ошибки замените 3.11 питон на 3.10

Комплексное решение вопроса описано тут: https://t.me/stable_cascade_rus/47/2536
